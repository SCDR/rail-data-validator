...
import {
    Card, Button, Row, Col, Statistic, List, Descriptions, Tag, Divider, Alert, Space, Typography, Popover,
    message,
    Radio,
    Progress,
    Input
...
import {
    CheckCircleOutlined, WarningOutlined, ExclamationCircleOutlined, ArrowLeftOutlined, ArrowRightOutlined, RestOutlined
...
// 中英文映射（复用原有逻辑）
const chMap = new Map([
    ['ExtraCol1', '额外列1'], ['ExtraCol2', '额外列2'], ['SlopeEndCol', '顺坡终点'],
    ['SwitchTipCol', '尖轨尖'], ['SwitchMiddleCol', '尖轨中'], ['SwitchHeelCol', '尖轨跟'],
    ['LeadCurveFrontCol', '导曲前'], ['LeadCurveMiddleCol', '导曲中'], ['LeadCurveRearCol', '导曲后'],
    ['FrogFrontCol', '辙岔前'], ['FrogMiddleCol', '辙岔中'], ['FrogRearCol', '辙岔后'],
    ['CheckIntervalCol', '查照间隔'], ['GuardDistanceCol', '护背距离']
]);
...
interface RailDataValidatorProps {
    ...
    onDataChange?: (type: 'gauge' | 'horizontal', newData: Record<string, string>) => void;
...
const RailDataCollector: React.FC<RailDataValidatorProps> = ({
    ...
    const [railStats, setRailStats] = useState<any>(null);
    const [horizontalStats, setHorizontalStats] = useState<any>(null);
    const [loading, setLoading] = useState(false);

    // ====================== 新增：录入相关状态 ======================
    const [currentInputType, setCurrentInputType] = useState<'gauge' | 'horizontal'>('gauge'); // 当前录入类型
    const [currentItemIndex, setCurrentItemIndex] = useState(0); // 当前录入项索引
    const [transitionDirection, setTransitionDirection] = useState<'next' | 'prev'>('next');
    const [gaugeInputData, setGaugeInputData] = useState<Record<string, string>>({}); // 轨距已录入数据
    const [horizontalInputData, setHorizontalInputData] = useState<Record<string, string>>({}); // 水平已录入数据
    ...
    const currentData = currentInputType === 'gauge' ? gaugeInputData : horizontalInputData; // 当前类型已录入数据
    const currentValue = currentData[currentItem.name] || ''; // 当前录入项的值

    // 计算录入进度（已填有效值的项数 / 总项数）
    const getInputProgress = (): number => {
        const completedCount = Object.values(currentData).filter(val => val.trim() !== '').length;
    ...
    const handleNextItem = () => {
        if (currentItemIndex >= totalItems - 1) {
            message.success(`✅ ${currentInputType === 'gauge' ? '轨距' : '水平'}数据录入完成！`);
            return;
        }
        setTransitionDirection('next');
        setCurrentItemIndex(prev => prev + 1);
    ...
    // 验证所有数据
    const validateAll = () => {
        if (Object.keys(gaugeInputData).length === 0 || Object.keys(horizontalInputData).length === 0) {
        ...
        RuleConfigurator.configureHorizontalRules(horizontalValidator, horizontalColumns);
    ...
    // ====================== 原有渲染函数（保留）======================
    const renderErrorStats = (stats: any, title: string) => {
                            ...
                            <Card size="small">
                                ...
                                <div style={{ marginTop: 8 }}>
                                    <Popover title="错误详情" content={
                                        <div>{data.errors.map((error: any) => <p key={error.message}>{error.message}</p>)}</div>
                                    }>
    ...
    const renderErrors = (errors: ValidationError[], title: string) => {
        if (errors.length === 0) {
            return (
                <Alert
    ...
    return (
        <div style={{ maxWidth: '100%', overflowX: 'hidden' }}>
            {/* 1. 录入区域（核心新增） */}
            <Card title="轨道数据逐项录入" bordered={false} style={{ marginBottom: 24, maxWidth: '100%' }}>
                {/* 1.1 录入类型切换 */}
                <Radio.Group
                    value={currentInputType}
                    onChange={(e) => setCurrentInputType(e.target.value as 'gauge' | 'horizontal')}
                ...
                <Progress
                    ...
                    showInfo
                    format={(percent) => `${percent}% (${currentItemIndex + 1}/${totalItems})`}
                    style={{ marginBottom: 24, width: '100%' }}
                />

                {/* 1.3 当前录入项（带动画过渡） */}
                <div className="transition-wrapper" style={{ position: 'relative', maxWidth: '100%', overflowX: 'hidden' }}>
                    <SwitchTransition mode="out-in">
                        <CSSTransition
                            nodeRef={transitionRef}
                            key={currentItemIndex}
                            timeout={300}
                            classNames={transitionDirection === 'next' ? 'input-transition-next' : 'input-transition-prev'}
                        >
                            <div ref={transitionRef} className="current-input-item" style={{ padding: 20, border: '1px solid #e8e8e8', borderRadius: 8, background: '#fff', width: '100%' }}>
                                {/* 录入项标签 */}
                                <Text strong style={{ fontSize: 16, marginBottom: 8, display: 'block' }}>
                                    当前录入：{currentItem.label}（{currentItemIndex + 1}/{totalItems}）
                                </Text>

                                {/* 输入框+设备填充按钮 */}
                                <Space.Compact style={{ width: '100%', marginBottom: 16, maxWidth: '100%' }}>
                                    <Input
                                        ref={inputRef}
                                        value={currentValue}
                                        onChange={(e) => updateCurrentData({ ...currentData, [currentItem.name]: e.target.value })}
