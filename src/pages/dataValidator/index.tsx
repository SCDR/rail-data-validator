...
const NoFoundPage: React.FC = () => {
  ...
  const onStepChange = (value: number) => {
    ...
    setTimeout(() => {
      const ref =
        value === 0 ? straightValidatorRef.current : curvedValidatorRef.current;
      if (ref) {
        ...
        switch (section) {
          case 'straightGauge':
            sessionStorage.setItem(
              ...
              JSON.stringify(toStringRecord(straightRailFormData.gauge)),
            );
            setIsStraightGaugeEditing(false);
            break;
          case 'straightHorizontal':
            sessionStorage.setItem(
              ...
              JSON.stringify(toStringRecord(straightRailFormData.horizontal)),
            );
            setIsStraightHorizontalEditing(false);
            break;
          case 'curvedGauge':
            sessionStorage.setItem(
              ...
              JSON.stringify(toStringRecord(curvedRailFormData.gauge)),
            );
            setIsCurvedGaugeEditing(false);
            break;
          case 'curvedHorizontal':
            sessionStorage.setItem(
              ...
              JSON.stringify(toStringRecord(curvedRailFormData.horizontal)),
            );
            setIsCurvedHorizontalEditing(false);
            break;
        }
...
`;
  ...
  const TriangleMarker: React.FC<{ onClick?: () => void; size?: number }> = ({
    onClick,
    size,
  }) => {
              ...
              <Space wrap>
                {!isStraightGaugeEditing && (
                  <Button
                    size="small"
                    type="primary"
                    onClick={() => toggleOverviewEdit('straightGauge')}
                  >
                    编辑
                  </Button>
                )}
                {isStraightGaugeEditing && (
                  <>
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => saveOverviewEdit('straightGauge')}
                    >
                      保存
                    </Button>
                    <Button
                      size="small"
                      danger
                      onClick={() => cancelOverviewEdit('straightGauge')}
                    >
                      取消
                    </Button>
                  </>
              ...
              >
                ...
                >
                  {straightGaugeColumnTypes
                    .filter((col) => !col.hidden)
                    .map((col) => {
                      const errors = getColumnErrors(
                        col.name,
                        straightGaugeErrors,
                      );
                      const hasError = errors.length > 0;
                      const hasTriangle = errors.some(
                      ...
                      return (
                        ...
                        >
                          <Flex
                            id={`item-straightGauge-${col.name}`}
                          ...
                          >
                            {!isStraightGaugeEditing && (
                              <span data-connector-anchor="value">
                                {straightRailFormData.gauge[col.name] || ''}
                              </span>
                            )}
                            {isStraightGaugeEditing && (
                              <Input
                ...
                <ConnectorLayer
                  ...
                  pairs={getTrianglePairsFromErrors(straightGaugeErrors)}
                />
              </div>
                ...
                <Alert
                  ...
                  description={
                    <ul style={{ paddingLeft: 16, margin: 0 }}>
                      {getRowErrors(straightGaugeErrors).map((error, _idx) => (
                        <li
                        ...
                        >
                          <div style={{ color: '#ff4d4f', fontWeight: 500 }}>
                            {(error.columnNames ?? [])
                              .map((n: string) => {
                                const found = straightGaugeColumnTypes.find(
                                  (c) => c.name === n,
                                );
        ...
        (isHorizontalComplete ? (
          <Card
            ...
            headStyle={{
              fontSize: '1.1rem',
              fontWeight: 'bold',
              backgroundColor: isStraightHorizontalEditing
                ? '#fffbe6'
                : '#e6f7ff',
              padding: '12px 24px',
            }}
              ...
              <Space wrap>
                {!isStraightHorizontalEditing && (
                  <Button
                    size="small"
                    type="primary"
                    onClick={() => toggleOverviewEdit('straightHorizontal')}
                  >
                    编辑
                  </Button>
                )}
                {isStraightHorizontalEditing && (
                  <>
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => saveOverviewEdit('straightHorizontal')}
                    >
                      保存
                    </Button>
                    <Button
                      size="small"
                      danger
                      onClick={() => cancelOverviewEdit('straightHorizontal')}
                    >
                      取消
                    </Button>
                  </>
          ...
          >
            <Space direction="vertical">
              <div
                ref={straightHorizontalOverviewRef}
                style={{ position: 'relative' }}
              >
                <Descriptions
                  bordered
                  ...
                  {straightHorizontalColumnTypes
                    ...
                    .map((col) => {
                      ...
                      return (
                        ...
                        >
                          <Flex
                            id={`item-straightHorizontal-${col.name}`}
                            justify={'space-around'}
                            wrap
                            gap="small"
                          >
                            {!isStraightHorizontalEditing && (
                              <span data-connector-anchor="value">
                                {straightRailFormData.horizontal[col.name] ||
                                  ''}
                              </span>
                            )}
                            {isStraightHorizontalEditing && (
                ...
                <ConnectorLayer
                  ...
                  pairs={getTrianglePairsFromErrors(straightHorizontalErrors)}
                />
              </div>
              ...
              {getRowErrors(straightHorizontalErrors).length > 0 && (
                <Alert
                  ...
                  description={
                    <ul style={{ paddingLeft: 16, margin: 0 }}>
                      {getRowErrors(straightHorizontalErrors).map(
                        (error, _idx) => (
                          <li
                            key={`${error.columnNames?.join('|') || 'cols'}-${String(error.ruleName || error.message || 'row')}`}
                            style={{ marginBottom: 8, listStyle: 'disc' }}
                          >
                            <div style={{ color: '#ff4d4f', fontWeight: 500 }}>
                              {(error.columnNames ?? [])
                                .map((n: string) => {
                                  const found =
                                    straightHorizontalColumnTypes.find(
                                      (c) => c.name === n,
                                    );
                                  return found ? found.label : n;
                                })
              ...
              <Space wrap>
                {!isCurvedGaugeEditing && (
                  <Button
                    size="small"
                    type="primary"
                    onClick={() => toggleOverviewEdit('curvedGauge')}
                  >
                    编辑
                  </Button>
                )}
                {isCurvedGaugeEditing && (
                  <>
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => saveOverviewEdit('curvedGauge')}
                    >
                      保存
                    </Button>
                    <Button
                      size="small"
                      danger
                      onClick={() => cancelOverviewEdit('curvedGauge')}
                    >
                      取消
                    </Button>
                  </>
            ...
            <Space direction="vertical">
              ...
              >
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {!isCurvedGaugeEditing && (
                              <span data-connector-anchor="value">
                                {curvedRailFormData.gauge[col.name] || ''}
                              </span>
                            )}
                            {isCurvedGaugeEditing && (
                              <Input
                            ...
                            {hasTriangle && (
                              <TriangleMarker
                                onClick={() =>
                                  scrollToOverviewItem('curvedGauge', col.name)
                                }
                              />
                            )}
                ...
                <ConnectorLayer
                  ...
                  section="curvedGauge"
                  pairs={getTrianglePairsFromErrors(curvedGaugeErrors)}
                />
              </div>
              ...
              {getRowErrors(curvedGaugeErrors).length > 0 && (
                <Alert
                  ...
                  description={
                    <ul style={{ paddingLeft: 16, margin: 0 }}>
                      {getRowErrors(curvedGaugeErrors).map((error, _idx) => (
                        <li
                          key={`${error.columnNames?.join('|') || 'cols'}-${String(error.ruleName || error.message || 'row')}`}
                          style={{ marginBottom: 8, listStyle: 'disc' }}
                        >
                          <div style={{ color: '#ff4d4f', fontWeight: 500 }}>
                            {(error.columnNames ?? [])
                              .map((n: string) => {
                                const found = curvedGaugeColumnTypes.find(
                                  (c) => c.name === n,
                                );
                                return found ? found.label : n;
                              })
        ...
        (isHorizontalComplete ? (
          <Card
            ...
            headStyle={{
              fontSize: '1.1rem',
              fontWeight: 'bold',
              backgroundColor: isCurvedHorizontalEditing
                ? '#fffbe6'
                : '#e6f7ff',
              padding: '12px 24px',
            }}
              ...
              <Space wrap>
                {!isCurvedHorizontalEditing && (
                  <Button
                    size="small"
                    type="primary"
                    onClick={() => toggleOverviewEdit('curvedHorizontal')}
                  >
                    编辑
                  </Button>
                )}
                {isCurvedHorizontalEditing && (
                  <>
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => saveOverviewEdit('curvedHorizontal')}
                    >
                      保存
                    </Button>
                    <Button
                      size="small"
                      danger
                      onClick={() => cancelOverviewEdit('curvedHorizontal')}
                    >
                      取消
                    </Button>
                  </>
          ...
          >
            <div
              ref={curvedHorizontalOverviewRef}
              style={{ position: 'relative' }}
            >
              <Descriptions
                bordered
                ...
                {curvedHorizontalColumnTypes
                  ...
                  .map((col) => {
                    ...
                    return (
                      ...
                      >
                        <Flex
                          id={`item-curvedHorizontal-${col.name}`}
                          justify={'space-around'}
                          wrap
                          gap="small"
                        >
                          {!isCurvedHorizontalEditing && (
                            <span data-connector-anchor="value">
                              {curvedRailFormData.horizontal[col.name] || ''}
                            </span>
                          )}
                          {isCurvedHorizontalEditing && (
                            <Input
              ...
              <ConnectorLayer
                ...
                pairs={getTrianglePairsFromErrors(curvedHorizontalErrors)}
              />
            </div>
            ...
            {getRowErrors(curvedHorizontalErrors).length > 0 && (
              <Alert
                ...
                description={
                  <ul style={{ paddingLeft: 16, margin: 0 }}>
                    {getRowErrors(curvedHorizontalErrors).map((error, _idx) => (
                      <li
                        key={`${error.columnNames?.join('|') || 'cols'}-${String(error.ruleName || error.message || 'row')}`}
                        style={{ marginBottom: 8, listStyle: 'disc' }}
                      >
                        <div style={{ color: '#ff4d4f', fontWeight: 500 }}>
                          {(error.columnNames ?? [])
                            .map((n: string) => {
                              const found = curvedHorizontalColumnTypes.find(
                                (c) => c.name === n,
                              );
                              return found ? found.label : n;
                            })
              ...
              <Space direction="vertical">
                ...
                {getRowErrors(straightGaugeErrors).length > 0 && (
                  <Alert
                    ...
                    description={
                      <ul style={{ paddingLeft: 16, margin: 0 }}>
                        {getRowErrors(straightGaugeErrors).map(
                          (error, _idx) => (
                            <li
                              key={`${error.columnNames?.join('|') || 'cols'}-${String(error.ruleName || error.message || 'row')}`}
                              style={{ marginBottom: 8, listStyle: 'disc' }}
            ...
            >
              ...
              {getRowErrors(curvedHorizontalErrors).length > 0 && (
                <Alert
                  ...
                  description={
                    <ul style={{ paddingLeft: 16, margin: 0 }}>
                      {getRowErrors(curvedHorizontalErrors).map(
                        (error, _idx) => (
                          <li
                            key={`${error.columnNames?.join('|') || 'cols'}-${String(error.ruleName || error.message || 'row')}`}
                            style={{ marginBottom: 8, listStyle: 'disc' }}
                          >
                            <div style={{ color: '#ff4d4f', fontWeight: 500 }}>
                              {(error.columnNames ?? [])
                                .map((n: string) => {
                                  const found =
                                    curvedHorizontalColumnTypes.find(
                                      (c) => c.name === n,
                                    );
                                  return found ? found.label : n;
                                })
