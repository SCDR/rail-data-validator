...
import { ColumnType, DataRow, ValidationError } from '@/components/Validation/types';
...
const railsDistanceColumnTypes = [
 ...
 */
const RailDataValidator = forwardRef<RailDataValidatorHandle, RailDataValidatorProps>(({
    gaugeData,           // 轨距数据
    horizontalData,     // 水平数据
    ...
    // 回退数据与状态
    const [fallbackGaugeData, setFallbackGaugeData] = useState<DataRow[] | null>(null);
    const [fallbackHorizontalData, setFallbackHorizontalData] = useState<DataRow[] | null>(null);
    const [fallbackStatus, setFallbackStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
    ...
    useEffect(() => {
        ...
        try {
            ...
            // 先尝试直轨
            if (needGauge) {
                const sg = sessionStorage.getItem(STORAGE_KEYS.straightGauge) || sessionStorage.getItem(STORAGE_KEYS.curvedGauge);
                if (sg) {
            ...
            if (needHorizontal) {
                const sh = sessionStorage.getItem(STORAGE_KEYS.straightHorizontal) || sessionStorage.getItem(STORAGE_KEYS.curvedHorizontal);
                if (sh) {
    ...
    // 选用有效数据
    const effectiveGaugeData: DataRow[] = (fallbackGaugeData ?? gaugeData) || [];
    const effectiveHorizontalData: DataRow[] = (fallbackHorizontalData ?? horizontalData) || [];
    ...
    const isGaugeComplete = React.useMemo(() => isDataComplete(effectiveGaugeData, gaugeColumns), [effectiveGaugeData, gaugeColumns]);
    const isHorizontalComplete = React.useMemo(() => isDataComplete(effectiveHorizontalData, horizontalColumns), [effectiveHorizontalData, horizontalColumns]);

    // 状态管理
    const [railErrors, setRailErrors] = useState<ValidationError[]>([]);      // 轨距数据错误列表
    const [horizontalErrors, setHorizontalErrors] = useState<ValidationError[]>([]); // 水平数据错误列表
    ...
    const validateRailData = async (): Promise<ValidationError[]> => {
        ...
        if (!isGaugeComplete) {
            ...
            setRailStats(null);
            setLoading(false);
            try { onValidationComplete([], horizontalErrors); } catch {}
            return [];
        }
        ...
        setLoading(false);
        try { onValidationComplete(errors, horizontalErrors); } catch {}
        return errors;
    };
    ...
    const validateHorizontalData = async (): Promise<ValidationError[]> => {
        ...
        if (!isHorizontalComplete) {
            ...
            setLoading(false);
            try { onValidationComplete(railErrors, []); } catch {}
            return [];
        }
        const validator = new DataValidator();
        ...
        setLoading(false);
        try { onValidationComplete(railErrors, errors); } catch {}
        return errors;
    };
     ...
     * 同时验证轨距与水平数据（异步）
     */
    const validateAll = async (): Promise<{ railErrors: ValidationError[], horizontalErrors: ValidationError[] }> => {
        setLoading(true);
        const railValidator = new DataValidator();
        RuleConfigurator.configureRailRules(railValidator, gaugeColumns);
        const railErrors1 = isGaugeComplete ? railValidator.validateAll(effectiveGaugeData) : [];
        ...
        RuleConfigurator.configureHorizontalRules(horizontalValidator1, horizontalColumns);
        const horizontalErrors1 = isHorizontalComplete ? horizontalValidator1.validateAll(effectiveHorizontalData) : [];
        setHorizontalErrors(horizontalErrors1);
        setHorizontalStats(isHorizontalComplete ? horizontalValidator1.getErrorStatistics() : null);

        try { onValidationComplete(railErrors1, horizontalErrors1); } catch {}
        setLoading(false);
    ...
    const gaugeLabelMap = React.useMemo(() => new Map(gaugeColumns.map(c => [c.name, (c as any).label || c.name])), [gaugeColumns]);
    const horizontalLabelMap = React.useMemo(() => new Map(horizontalColumns.map(c => [c.name, (c as any).label || c.name])), [horizontalColumns]);
    const renderErrorStats = (stats: any, title: string, labelMap: Map<string, string>) => {
        if (!stats) return null;

        return (
            <Card title={`${title}错误统计`} bordered={false} style={{ marginTop: 16 }}>
                <Row gutter={16}>
                    <Col span={12}>
                        <Statistic
                            title="总错误数"
                ...
                <Row gutter={[16, 16]}>
                    {Object.entries(stats.byColumn).map(([column, data]: [string, any]) => (
                        <Col
                            key={column}
                            xs={12}
                            sm={12}
                            ...
                            <Card size="small">
                                ...
                                <div style={{ marginTop: 8 }}>
                                    <Popover title="错误详情" content={
                                        ...
                                        <Tag color="error"><ExclamationCircleOutlined /> {data.count}</Tag>
                                    </Popover>
    ...
    const renderErrors = (errors: ValidationError[], title: string, isComplete: boolean, labelMap: Map<string, string>) => {
        if (errors.length === 0) {
        ...
        return (
            <Collapse defaultActiveKey={[]} style={{ marginTop: 16 }}>
                <Collapse.Panel header={`${title}详细错误列表`} key={`${title}-detail-errors`} >
                    <div style={{ height: 320, overflowY: 'auto', paddingRight: 8 }}>
                        <List
                            ...
                            renderItem={(error, index) => {
                                const isTriangleDepression = error.ruleName.includes('Triangle_Depression');
                                return (
                                    <List.Item>
                                        <List.Item.Meta
                                            avatar={
                                            ...
                                            title={
                                                <Space>
                                                    ...
                                                    <Text strong>{error.columnNames.map((n: string) => labelMap.get(n) || n).join(' & ')}</Text>
                                                </Space>
                                            }
                                            description={
                                                <div>
    ...
    return (
        <Card title="数据校验摘要" bordered={false} style={{ width: '100%' }}>
            ...
            {fallbackStatus === 'loading' && (
                <Alert showIcon type="info" message="正在尝试从会话存储读取数据回退..." style={{ marginBottom: 16 }} />
            )}
            {fallbackStatus === 'error' && (
                <Alert showIcon type="error" message="会话数据回退失败" description={fallbackError || undefined} style={{ marginBottom: 16 }} />
            )}
            {fallbackStatus === 'success' && (
                <Alert showIcon type="success" message="已加载会话数据回退" style={{ marginBottom: 16 }} />
            )}
                ...
                >
                    <ProCard
                        ...
                        extra={
                            <Space>
                                <Button
                                    onClick={validateRailData}
                                    loading={loading}
                                >
                                    校验轨距数据
                                </Button>
                                {typeof onToggleEdit === 'function' && typeof onSaveEdit === 'function' && typeof onCancelEdit === 'function' && (
                                    !isEditing ? (
                                        <Button type="primary" size="small" onClick={onToggleEdit}>编辑</Button>
                                    ) : (
                                        <>
                                            <Button type="primary" size="small" onClick={onSaveEdit}>保存</Button>
                                            <Button danger size="small" onClick={onCancelEdit}>取消</Button>
                                        </>
                                    )
                                )}
                            </Space>
                    ...
                    >
                        <Row>
                            <Col span={24}>
                                {railStats && renderErrorStats(railStats, "轨距", gaugeLabelMap)}
                            </Col>
                            <Col span={24}>
                                {renderErrors(railErrors, "轨距", isGaugeComplete, gaugeLabelMap)}
                            </Col>
                        </Row>
                    </ProCard>
                </Col>
                ...
                >
                    <ProCard
                        ...
                        extra={
                            <Space>
                                <Button
                                    onClick={validateHorizontalData}
                                    loading={loading}
                                >
                                    校验水平数据
                                </Button>
                    ...
                    >
                        <Row>
                            <Col span={24}>
                                {horizontalStats && renderErrorStats(horizontalStats, "水平", horizontalLabelMap)}
                            </Col>
                            <Col span={24}>
                                {renderErrors(horizontalErrors, "水平", isHorizontalComplete, horizontalLabelMap)}
                            </Col>
                        </Row>
                    </ProCard>
                </Col>
...
export interface RailDataValidatorHandle {
    ...
    validateAll: () => Promise<{ railErrors: ValidationError[], horizontalErrors: ValidationError[] }>;
}